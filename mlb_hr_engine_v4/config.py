...
# ── Role Layer Thresholds (PRIME + EXPLOSIVE ticket roles) ───────────────────
# Display-only classification derived from existing MAIN row fields.
# Read-only: these flags are NEVER written back to model_prob, score, or sort keys.
# Source of truth for roles.py — do not scatter literals elsewhere.
#
# PRIME: reliable anchor role; requires top-tier model confidence + elite Statcast core.
# EXPLOSIVE: explosive-outcome candidate; requires raw power ceiling (max EV) + at least one
#            swing trait (blast% OR pull-air%) — null blast is not disqualifying (see §5).
ROLE_PRIME_BARREL_PCT:  float = 9.0   # barrel% ≥ this (string field, strip "%")
ROLE_PRIME_XSLG:        float = 0.500 # xSLG ≥ this
ROLE_PRIME_HH_PCT:      float = 45.0  # HH% ≥ this (string field, strip "%")
ROLE_PRIME_EV:          float = 90.0  # exit_velo ≥ this (mph)
ROLE_PRIME_TIERS: set   = frozenset({"APEX", "ELITE"})  # tier must be in this set

ROLE_EXPLOSIVE_MAX_EV:         float = 113.0 # max_ev ≥ this (mph)
ROLE_EXPLOSIVE_BARREL_PCT:     float = 8.5   # barrel% ≥ this (same parse as PRIME)
ROLE_EXPLOSIVE_BLAST_PCT:      float = 12.0  # blast% ≥ this (when present; null = skip sub-criterion)
ROLE_EXPLOSIVE_PULL_AIR_PCT:   float = 20.0  # pull_air_pct ≥ this (when present; null = skip sub-criterion)
...
# Shared top-tier gate for ADVANTAGE + WILDCARD exclusion.
ROLE_TOP_TIERS: set = frozenset({"APEX", "ELITE"})
