...
def print_model_probabilities(all_players: list[dict], top_n: int = 20) -> None:
    """Show top N players by model probability (works without odds)."""
    console.print(Panel(
    ...
    table.add_column("Exp PA",       min_width=6, justify="right", no_wrap=True)
    ...
    for p in sorted_players[:top_n]:
        ...
        spot_str = str(spot) if spot else "?"
